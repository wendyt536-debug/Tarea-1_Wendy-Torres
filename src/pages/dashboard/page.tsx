...
const CHART_COLORS = ["#16a34a", "#0ea5e9", "#f59e0b", "#e11d48", "#8b5cf6", "#14b8a6", "#f97316"];
...
export default function DashboardPage() {
  ...
  const filteredIntakes = useMemo(() => {
    if (lobFilter === "All") return intakes;
    return intakes.filter((i) => i.lineOfBusiness === lobFilter);
  }, [intakes, lobFilter]);

  const stats = useMemo(() => {
    const total = filteredIntakes.length;
    const closed = filteredIntakes.filter((i) =>
      ["Published", "Closed", "Completed", "Cancelled", "Transferred to US Team"].includes(i.status),
    );
    const open = filteredIntakes.filter(
      (i) => !["Published", "Closed", "Completed", "Cancelled", "Transferred to US Team"].includes(i.status),
    );
  ...
  const processMetrics = useMemo(() => {
    const filteredClosed = filteredIntakes.filter((i) =>
      ["Published", "Closed", "Completed", "Cancelled", "Transferred to US Team"].includes(i.status),
    );
    const filteredOpen = filteredIntakes.filter(
      (i) => !["Published", "Closed", "Completed", "Cancelled", "Transferred to US Team"].includes(i.status),
    );

    const completionTimes = filteredClosed
    ...
    const daysList = filteredOpen.map((i) => computeDaysInProcess(i));
    ...
    <AppLayout>
        ...
        actions={
          <div className="flex items-center gap-3">
            <select
              ...
              className="text-sm border border-slate-300 rounded-md px-3 py-2 bg-white text-slate-700 cursor-pointer min-w-[200px] focus:outline-none focus:ring-2 focus:ring-brand-200 focus:border-brand-300"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-lg border border-slate-200 p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-sm font-semibold text-slate-900">Intakes by Contract Type</h3>
